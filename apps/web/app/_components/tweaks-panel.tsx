...
export function TweaksPanel({ title = 'Tweaks', children }: TweaksPanelProps) {
  ...
  if (!open) return null;
  // Use static initial right/bottom — clampToViewport (on open) writes the
  // real values imperatively so we don't have to read `offsetRef.current`
  // during render.
  return (
    <>
...
export type TweakRadioOption<V> = V | { value: V; label: string };

export interface TweakRadioProps<V> {
  label: string;
  value: V;
...
export function TweakRadio<V extends string | number | boolean>({
  ...
  // Kept in sync with the latest prop so pointer-move handlers (attached
  // for the lifetime of a drag) don't see stale closures.
  ...
  const asObj = (o: TweakRadioOption<V>): { value: V; label: string } | null =>
    typeof o === 'object' && o !== null && 'value' in o
      ? (o as { value: V; label: string })
      : null;
  const labelLen = (o: TweakRadioOption<V>): number => {
    const obj = asObj(o);
    return String(obj ? obj.label : (o as V)).length;
  };
  const maxLen = options.reduce((m, o) => Math.max(m, labelLen(o)), 0);
  const segCap: Record<number, number> = { 2: 16, 3: 10 };
  const fitsAsSegments = maxLen <= (segCap[options.length] ?? 0);
  if (!fitsAsSegments) {
    const resolve = (s: string): V => {
      const m = options.find((o) => {
        const obj = asObj(o);
        return String(obj ? obj.value : (o as V)) === s;
      });
      if (m === undefined) return s as unknown as V;
      const obj = asObj(m);
      return (obj ? obj.value : (m as V)) as V;
    };
    return (
      <TweakSelect
        label={label}
  ...
  const opts: { value: V; label: string }[] = options.map((o) => {
    const obj = asObj(o);
    return obj || { value: o as V, label: String(o) };
  });
  const idx = Math.max(
    0,
...
export interface TweakSelectProps<V> {
  label: string;
  value: V;
  options: TweakRadioOption<V>[];
...
export function TweakSelect<V>({
  ...
  return (
    <TweakRow label={label}>
      ...
      >
        {options.map((o) => {
          const isObj = typeof o === 'object' && o !== null && 'value' in o;
          const opt = isObj ? (o as { value: V; label: string }) : null;
          const v = opt ? opt.value : (o as V);
          const l = opt ? opt.label : (o as unknown as string);
          return (
...
export interface TweakColorProps {
  ...
  onChange: (next: TweakColorValue) => void;
}
        ...
        {options.map((o, i) => {
          ...
          return (
            <button
              ...
              title={colors.join(' · ')}
              style={{ background: hero } satisfies CSSProperties}
              onClick={() => onChange(o)}
            >
              {sup.length > 0 && (
                <span>
